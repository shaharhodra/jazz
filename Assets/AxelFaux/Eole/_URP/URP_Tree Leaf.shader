	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float _wind_amplitude116_g3676 = WindAmplitude;
				float3 ase_worldPos = TransformObjectToWorld( (v.positionOS).xyz );
				float3 worldToObj8_g3675 = mul( GetWorldToObjectMatrix(), float4( ase_worldPos, 1 ) ).xyz;
				float3 worldToObj43_g3675 = mul( GetWorldToObjectMatrix(), float4( v.positionOS.xyz, 1 ) ).xyz;
				float mulTime45_g3675 = _TimeParameters.x * _TreeBendFrequencySpeed;
				float4 transform69_g3675 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				float temp_output_4_0_g3675 = ( _wind_amplitude116_g3676 * saturate( ( ( worldToObj8_g3675.y - _TreeBendMaskDistanceOffset ) / _TreeBendMaskFalloff ) ) * (-_TreeBendMinAngle + (cos( ( ( ( worldToObj43_g3675.y + mulTime45_g3675 ) * _TreeBendFrequency ) + length( ( transform69_g3675 * _TreeBendFrequencyOffsetRandomn ) ) ) ) - -1.0) * (-_TreeBendMaxAngle - -_TreeBendMinAngle) / (1.0 - -1.0)) );
				...
				float3 worldToObj35_g3675 = mul( GetWorldToObjectMatrix(), float4( ase_worldPos, 1 ) ).xyz;
				float3 rotatedValue2_g3675 = RotateAroundAxis( float3( 0,0,0 ), ( wpo_out_turbulence508_g3670 + worldToObj35_g3675 ), normalize( cross( wpo_wind_direction306_g3670 , float3( 0,1,0 ) ) ), temp_output_4_0_g3675 );
				#ifdef _DEBUGDISABLEWINDDPO_ON
				float3 staticSwitch466_g3670 = ( debug_disableWPO486_g3670 == 1.0 ? out_wpo_crush432_g3670 : ( rotatedValue2_g3675 - worldToObj35_g3675 ) );
				#else
				float3 staticSwitch466_g3670 = ( rotatedValue2_g3675 - worldToObj35_g3675 );
				#endif
...
ASEEND*/
//CHKSM=F3A674C796D950A8AC84DA1AC631494200B86716